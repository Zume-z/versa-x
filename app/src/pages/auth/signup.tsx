...
export default function SignIn() {
  ...
  const signInOAuth = async (provider: string) => {
    setLoading(true)
    await signIn(provider, { callbackUrl: VERSA_HREF.DASHBOARD }).then((res) => {
      setTimeout(() => {
        setLoading(false)
      }, 2000)
    })
              ...
              {providers && (
                <div className="flex h-full w-full items-center justify-center ">
                  <div className="w-full max-w-sm flex-col space-y-10  px-4 sm:-mt-10 sm:px-8">
                    <div className="w-full text-center text-2xl text-white">Create you account.</div>
                    ...
                    <div className="space-y-3">
                      <div className=" flex w-full items-center justify-center space-x-1 text-center text-sm">
                        ...
                        <p onClick={() => push('/auth/signin')} className="cursor-pointer text-blue-500 hover:underline">
                          Log in
                        </p>
                      </div>
